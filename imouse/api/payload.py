...
class Payload:
    ...
    def device_get(self, device_id: str = '', data: Optional[dict] = None) -> dict:
        return self._build_payload("/device/get", {"id": device_id}, data)

    def device_group_get(self, group_id: str = '', data: Optional[dict] = None) -> dict:
        return self._build_payload("/device/group/get", {"id": group_id}, data)

    def device_group_getdev(self, group_id: str, data: Optional[dict] = None) -> dict:
        return self._build_payload("/device/group/get-dev", {"id": group_id}, data)

    def device_set(self, device_id: str, params: SetDeviceParams, data: Optional[dict] = None) -> dict:
        return self._build_payload("/device/set", {"id": device_id, **dict(params)}, data)

    def device_del(self, device_id: str, data: Optional[dict] = None) -> dict:
        return self._build_payload("/device/del", {"id": device_id}, data)

    def device_group_set(self, group_id: str, group_name: str, data: Optional[dict] = None) -> dict:
        return self._build_payload("/device/group/set", {"id": group_id, "name": group_name}, data)

    def device_group_del(self, group_id: str, data: Optional[dict] = None) -> dict:
        return self._build_payload("/device/group/del", {"id": group_id}, data)

    def device_airplay_set(self, device_id: str, params: SetDeviceAirplayParams, data: Optional[dict] = None) -> dict:
        return self._build_payload("/device/airplay/set", {"id": device_id, **dict(params)}, data)

    def device_airplay_connect(self, device_id: str, data: Optional[dict] = None) -> dict:
    ...
    def device_airplay_disconnect(self, device_id: str, data: Optional[dict] = None) -> dict:
        return self._build_payload("/device/airplay/disconnect", {"id": device_id}, data)

    def device_restart(self, device_id: str, data: Optional[dict] = None) -> dict:
        return self._build_payload("/device/restart", {"id": device_id}, data)

    def device_usb_restart(self, vid_pid: str, data: Optional[dict] = None) -> dict:
        return self._build_payload("/device/usb/restart", {"id": vid_pid}, data)

    def device_sort_set(self, sort_index, sort_value: int, data: Optional[dict] = None) -> dict:
    ...
    def config_imserver_set(self, params: ImServerConfigData, data: Optional[dict] = None) -> dict:
    ...
    def config_user_login(self, account_number, password: str, utag: int = 1, data: Optional[dict] = None) -> dict:
    ...
    def config_user_switch(self, utag: int, data: Optional[dict] = None) -> dict:
        return self._build_payload("/config/user/switch", {"id": utag}, data)

    def mouse_click(self, device_id, button: str, x, y, time: int, data: Optional[dict] = None) -> dict:
        return self._build_payload('/mouse/click', {"id": device_id, "button": button, "x": x, "y": y, "time": time},
                                   data)

    def mouse_swipe(self, device_id: str, params: MouseSwipeParams, data: Optional[dict] = None) -> dict:
        return self._build_payload('/mouse/swipe', {"id": device_id, **dict(params)}, data)

    def mouse_up(self, device_id, button: str, data: Optional[dict] = None) -> dict:
        return self._build_payload('/mouse/up', {"id": device_id, "button": button}, data)

    def mouse_down(self, device_id, button: str, data: Optional[dict] = None) -> dict:
        return self._build_payload('/mouse/down', {"id": device_id, "button": button}, data)

    def mouse_move(self, device_id: str, x, y: int, data: Optional[dict] = None) -> dict:
    ...
    def mouse_wheel(self, device_id, direction: str, len, number: int, data: Optional[dict] = None) -> dict:
        return self._build_payload('/mouse/wheel',
                                   {"id": device_id, "direction": direction, "len": len, "number": number}, data)

    def key_sendkey(self, device_id, key, fn_key: str, data: Optional[dict] = None) -> dict:
        return self._build_payload('/key/sendkey',
                                   {"id": device_id, "key": key, "fn_key": fn_key}, data)

    def key_sendhid(self, device_id: str, command_list: List[SendHidParams], data: Optional[dict] = None) -> dict:
        return self._build_payload('/key/sendhid',
                                   {"id": device_id, "list": command_list}, data)

    def pic_screenshot(self, device_id: str, is_jpg: bool, rect: Optional[List[int]], data: Optional[dict] = None) -> dict:
        return self._build_payload('/pic/screenshot',
                                   {"id": device_id, "jpg": is_jpg, "rect": rect, "binary": True}, data)

    def pic_find_image(self, device_id: str, img_list: List[str], similarity: float, all: bool, rect: Optional[List[int]],
    ...
    def pic_find_image_cv(self, device_id: str, img_list: List[str], similarity: float, all: bool, same: bool,
    ...
    def pic_ocr(self, device_id: str, rect: Optional[List[int]], is_ex: bool,
                data: Optional[dict] = None) -> dict:
        fun_str = "/pic/ocr-ex" if is_ex else "/pic/ocr"
        return self._build_payload(fun_str, {"id": device_id, "rect": rect}, data)

    def pic_find_text(self, device_id: str, text_list: List[str], similarity: float, contain: bool, rect: Optional[List[int]],
                      is_ex: bool,
                      data: Optional[dict] = None) -> dict:
    ...
    def pic_find_multi_color(self, device_id: str, params: List[MultiColorParams], all: bool, same: bool,
                             data: Optional[dict] = None) -> dict:
