...
class Pic():
    ...
    def pic_screenshot(self, device_id: str, is_jpg: bool = False, rect: Optional[List[int]] = None) -> Union[
        CommonResponse, bytes, None]:
    ...
    def pic_find_image(self, device_id: str, img_list: List[str], similarity: float = 0.85, all: bool = False,
                       ...
                       delta_color: str = "", direction: str = "", ) -> Optional[FindImageResultResponse]:
        ...
        return parse_model(FindImageResultResponse, ret) if isinstance(ret, dict) else None

    def pic_find_image_cv(self, device_id: str, img_list: List[str], similarity: float = 0.85, all: bool = False,
                          same: bool = False,
                          rect: Optional[List[int]] = None) -> Optional[FindImageCvResultResponse]:
        ...
        return parse_model(FindImageCvResultResponse, ret) if isinstance(ret, dict) else None

    def pic_ocr(self, device_id: str, rect: Optional[List[int]] = None, is_ex: bool = False,
        ...
        return parse_model(OcrResultResponse, ret) if isinstance(ret, dict) else None
    ...
    def pic_find_multi_color(self, device_id: str, params: List[MultiColorParams], all: bool = False,
                             same: bool = False
                             ) -> Optional[FindMultiColorResponse]:
        ...
        return parse_model(FindMultiColorResponse, ret) if isinstance(ret, dict) else None
